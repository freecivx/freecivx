...
public class MetaserverClient {
    private static final Logger logger = LoggerFactory.getLogger(MetaserverClient.class);
    ...
    public static void publishToMetaserver(int port) {
        ...
        try {
            ...
            logger.debug("Response: {}", connection.getResponseMessage());
            if (responseCode == HttpURLConnection.HTTP_OK) {
                logger.info("Server successfully published to metaserver.");
            } else {
                logger.error("Failed to publish to metaserver. Response code: {}", responseCode);
            }
        } catch (IOException e) {
            logger.error("Error publishing to metaserver: {}", e.getMessage(), e);
        }
    }
}
