...
map_handle_key(keyboard_key, key_code, ctrl, alt, shift, the_event)
{
  ...
  // - For hex maps (both C Freeciv server and freecivx-server): directions are rotated 45 degrees
  //   counterclockwise to match the 3D camera perspective, which views the map from the SE direction.
  //   Valid hex movements are NW, NE, E, SE, SW, W (not directly N or S).
  ...
  if (is_hex()) {
    ...
    // Square map: direct numpad directions (no rotation)
    switch (key_code) {
      case 35: // Numpad 1 (End) - down-left
      case 97:
        key_unit_move(DIR8_SOUTHWEST);
        break;

      case 40: // Numpad 2 (Down arrow) - down
      case 98:
        key_unit_move(DIR8_SOUTH);
        break;

      case 34: // Numpad 3 (Page Down) - down-right
      case 99:
        key_unit_move(DIR8_SOUTHEAST);
        break;

      case 37: // Numpad 4 (Left arrow) - left
      case 100:
        key_unit_move(DIR8_WEST);
        break;

      case 39: // Numpad 6 (Right arrow) - right
      case 102:
        key_unit_move(DIR8_EAST);
        break;

      case 36: // Numpad 7 (Home) - up-left
      case 103:
        key_unit_move(DIR8_NORTHWEST);
        break;

      case 38: // Numpad 8 (Up arrow) - up
      case 104:
        key_unit_move(DIR8_NORTH);
        break;

      case 33: // Numpad 9 (Page Up) - up-right
      case 105:
        key_unit_move(DIR8_NORTHEAST);
        break;
    }
...
function check_request_goto_path()
{
  if (goto_active && current_focus.length > 0) {
    ...
    } else if (prev_mouse_x == mouse_x && prev_mouse_y == mouse_y) {
      /* Server-side goto for hex maps: request when mouse stops. */
      clear_goto_tiles();
      if (ptile != null) {
...
function update_active_units_dialog()
{
  ...
  if (current_focus.length > 0) {
    ...
    var newwidth = 80 + punits.length * (width * 1.5);
