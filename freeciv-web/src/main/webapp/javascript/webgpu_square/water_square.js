...
function createWaterMaterialSquareTSL() {
  ...
  const { uniform, uv, vec2, vec3, vec4, sin, mix, fract, clamp, pow,
          mul, add, sub, abs, floor, texture, max, div,
          time, mx_noise_float } = THREE;
  
  // time is a built-in Three.js TSL node – no manual update needed
  const uvNode = uv();
...
****************************************************************************/
function updateWaterAnimationSquare() { /* driven by THREE.time node */ }
window.updateWaterAnimationSquare = updateWaterAnimationSquare;
window.add_quality_dependent_objects_webgpu_square = add_quality_dependent_objects_webgpu_square;
