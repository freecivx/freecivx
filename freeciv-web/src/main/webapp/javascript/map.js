 ...
 *
 * Valid directions depend on topology:
 *   - iso-hex (TF_HEX && TF_ISO): N, E, S, W, SE, NW (NE, SW invalid)
 *   - hex (!TF_ISO): N, E, S, W, NE, SW (SE, NW invalid)
 *   - square: all 8 directions valid
 *
 * This matches the Freeciv C server implementation in common/map.c
 */
...
/****************************************************************************
  ...
  This matches the Freeciv C server logic from map.c is_valid_dir_calculate():
    - For iso-hex (TF_HEX && TF_ISO): NE and SW are invalid
    - For non-iso hex (TF_HEX && !TF_ISO): SE and NW are invalid
    - N, E, S, W are always valid
    - For square maps, all 8 directions are valid
****************************************************************************/
function is_valid_dir(dir)
{
  ...
  case DIR8_SOUTHEAST:
  case DIR8_NORTHWEST:
    /* These directions are invalid in hex (non-iso) topologies. */
...
/****************************************************************************
  ...
  just a vertex. This matches the Freeciv C server logic.
  
  For iso-hex topologies: N, E, S, W, SE, NW are cardinal
  For non-iso hex topologies: N, E, S, W, NE, SW are cardinal
  For square maps: N, E, S, W are cardinal
****************************************************************************/
function is_cardinal_dir(dir)
{
  ...
  case DIR8_NORTHEAST:
  case DIR8_SOUTHWEST:
    /* These directions are cardinal in hexagonal (non-iso) topologies. */
    return topo_has_flag(TF_HEX) && !topo_has_flag(TF_ISO);
  }
  return false;
}
...
/****************************************************************************
  ...
  For hexagonal topologies, some directions are invalid:
    - iso-hex (TF_HEX && TF_ISO): NE and SW are invalid
    - non-iso hex (TF_HEX && !TF_ISO): SE and NW are invalid
