...
// Camera handling for hexagonal maps
// Uses the hex coordinate conversion from maputil_hex.js

/****************************************************************************
  Point the camera to look at point x, y, z in Three.js coordinates (hex).
****************************************************************************/
function camera_look_at_hex(x, y, z)
{
  camera_current_x = x;
  camera_current_y = y;
  camera_current_z = z;
...
/**************************************************************************
  Centers the mapview around the given tile (hexagonal).
**************************************************************************/
function center_tile_mapcanvas_3d_hex(ptile)
{
  if (ptile != null) {
    if (slide_init) {
      enable_mapview_slide_3d_hex(ptile);
    } else {
      var pos = map_to_scene_coords_hex(ptile['x'], ptile['y']);
      camera_look_at_hex(pos['x'] - 50, 0, pos['y'] - 50);
      slide_init = true;
    }
  }
}
...
/**************************************************************************
  Centers camera on a city (hexagonal).
**************************************************************************/
function center_tile_city_hex(city)
{
  var ptile = city_tile(city);
  if (ptile != null) {
    var pos = map_to_scene_coords_hex(ptile['x'], ptile['y']);
    camera_look_at_hex(pos['x'] , 0, pos['y'] );
  }
}

/**************************************************************************
  Enabled sliding of the mapview to the given tile (hexagonal).
**************************************************************************/
function enable_mapview_slide_3d_hex(ptile)
{
  var pos_dest = map_to_scene_coords_hex(ptile['x'], ptile['y']);
...
/**************************************************************************
  Updates mapview slide, called once per frame (same for hex and square).
**************************************************************************/
function update_map_slide_3d_hex()
{
  ...
  camera_look_at_hex(camera_current_x + dx, 0, camera_current_z + dy);
  mapview_slide['prev'] = mapview_slide['i'];
}
