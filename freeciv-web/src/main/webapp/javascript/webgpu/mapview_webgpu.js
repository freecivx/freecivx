...
/****************************************************************************
 Update terrain shader animation time uniform. Called from render loop.
 The terrain shader now includes integrated water rendering with animation.
****************************************************************************/
function updateTerrainAnimation(deltaTime) {
  if (window.terrainTimeUniform && window.terrainTimeUniform.value !== undefined) {
    window.terrainTimeUniform.value += deltaTime;
  }
}
...
async function init_webgpu_mapview() {
  ...
  // Add objects to scene (water is now rendered as part of terrain shader)
  add_all_objects_to_scene();
  console.log("Unified terrain mesh with integrated water rendering enabled.");
